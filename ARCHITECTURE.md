...
Zustand хранит только lifecycle auth-сессии: access token, initialize/logout и
pending flags. Access token дублируется в `sessionStorage` только до срока JWT
`exp`; refresh token никогда не доступен JavaScript и остаётся в httpOnly cookie.
